...
def epsilon_action(state, epsilon, q_table, player, episode):
    ...
    if np.random.random() < epsilon:
        if player == 1:
            return np.argmax(q_table[get_ternanry_conversion(state), :]), epsilon
        else:
            return np.argmin(q_table[get_ternanry_conversion(state), :]), epsilon
    else:
        return random.choice(actions_possible), epsilon
